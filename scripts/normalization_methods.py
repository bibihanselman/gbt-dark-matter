...
def inject_calibrated_v2(name, sig_locs, info, data_dir=None, save_dir=None, signal_sizes=1e-28, is_decay=True, template=False):
...
def inject_calibrated_xband(name, sig_locs, info, data_dir=None, save_dir=None, calibration_dir=None, signal_sizes=1e-28, is_decay=True, template=False):
    freq, spec = np.load(os.path.join(data_dir, name))
...
# Weighted normalization for calibrated spectra  
def normalize_weighted_v2(
        data, 
        data_dir=None, 
        save_dir=None, 
        a=0.1, 
    ...
    for x in xs_trimmed:
        ...
        current_ys = ys[i-window//2:i+window//2+1]
        idx = np.isfinite(current_ys)
