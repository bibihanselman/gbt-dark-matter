...
from numba import jit, njit, prange, set_num_threads, objmode
...
def inject_calibrated(
    ...
    '''General routine for signal injection.'''
    freq, spec = np.load(os.path.join(data_dir, name))
    
    targ_info = info.set_index('time')
    ...
    if template:
        ...
        sefd = 10 * np.ones_like(spec)

    if not isinstance(sefd, (list, np.ndarray)):
        sefd = sefd * np.ones_like(spec)
...
@njit(cache=True)
def weighted_sum(asymmetry, freqs, state, spectra, freq_diff=0.18, factor=1e-3):
    def weight(i):
        numerator = np.sum(np.isfinite(spectra[:, i]))
        denominator = np.sum(np.isfinite(spectra[:, i]) & state == 1)
        if denominator == 0:
            return 0
    ...
    nan_mask = ~np.isnan(msds)
    msds, weights = msds[nan_mask], weights[nan_mask]
    w_sum = np.dot(msds, weights)
    return w_sum 

@njit(cache=True)
def objective_wammsd(state, spectra, template_spectra, mask, freqs=None, freq_diff=0.18, factor=1e-3):
...
@njit(cache=True)
def objective_weighted_sum(state, spectra, template_spectra, mask, freqs=None, freq_diff=0.18, factor=1e-3):
    '''The objective function.'''
    asymmetry = build_asymmetry(spectra, state, mask)
    template = build_asymmetry(template_spectra, state, mask)

    template_integral = np.sum(np.abs(template) ** 2)
    denom = weighted_sum(asymmetry, freqs, state, spectra, freq_diff=freq_diff, factor=factor) 
    return template_integral / denom

@njit(cache=True)
def objective_add(state, spectra, template_spectra, mask, freqs=None, freq_diff=0.18, factor=1e-3, beta=1e-4):
    '''The objective function.'''
    asymmetry = build_asymmetry(spectra, state, mask)
    template = build_asymmetry(template_spectra, state, mask)
    ...
    template_integral = np.sum(np.abs(template) ** 2)
    denom = weighted_sum(asymmetry, freqs, state, spectra, freq_diff=freq_diff, factor=factor) 
    return beta * template_integral - denom
...
def cross_entropy(
    ...
    for i in range(n_gen): #trange(n_gen):
        ...
        fs = np.asfortranarray(fs) # For faster probability calculations
...
def entropy(i, elite):
    total = 0.0
    for j in range(elite.shape[0]):
        total += elite[j, i]
    p = total / elite.shape[0]
    return 0 if ((p == 1) or (p == 0)) else -p * np.log(p) - (1 - p) * np.log(1 - p)

@njit(cache=True)
def conditional_entropy(i1, i2, elite):
    '''Computes H(X_i1 | X_i2)'''
    H = 0.0
    for x_i1 in np.arange(2):
        for x_i2 in np.arange(2):
            # Joint probability p(X_i1, X_i2)
            total = 0.0
            for j in range(elite.shape[0]):
                if (elite[j, i1] == x_i1) and (elite[j, i2] == x_i2):
                    total += 1
            p_joint = total / elite.shape[0]

            # Individual probability p(X_i1)
            total = 0.0
            for j in range(elite.shape[0]):
                if (elite[j, i1] == x_i1):
                    total += 1
            p_ind = total / elite.shape[0]

            # Entropy
            term = 0 if p_joint == 0 else -p_joint * np.log(p_joint / p_ind)
            H += term
...
def mimic(
    ...
    '''
    Implementation of MIMIC algorithm. See paper:
    https://papers.nips.cc/paper_files/paper/1996/file/4c22bd444899d3b6047a10b20a2f26db-Paper.pdf
    '''
    perfs = np.zeros(n_gen)
    x_best = np.zeros(n_pop, dtype=np.int64)
    f_best = 0.0
    ...
    # Generate vectors given inital prob. vector (uniform)
    xs = np.empty((n_pop, len(p0)), dtype=np.int64)
    for j in range(n_pop):
        for k in range(len(p0)):
            xs[j,k] = np.random.binomial(1, p0[k])
    ...
    for i in range(n_gen):
        with objmode():
            t = datetime.timedelta(seconds=time.perf_counter() - start_time)
            print(f'{i}/1000: {t}')
        ...
        # progress.set(0)
        print('Computing optimal permutation')

        # Compute optimal permutation - greedy algorithm
        # Step 1: minimum individual entropy
        pi = np.full(len(p0), -1, dtype=np.int64) # Permutation
        ents = np.zeros(len(p0), dtype=np.float64)
        for j in prange(len(p0)):
            ents[j] = entropy(j, elite)
        pi[0] = np.argmin(ents)
        # progress.update(1)
        # Step 2: conditional entropies
        for j in range(1, len(p0)):
            for k in prange(len(p0)):
                if k in pi:
                    ents[k] = np.inf
                else:
                    ents[k] = conditional_entropy(k, pi[j-1], elite)
            pi[j] = np.argmin(ents)
            # progress.update(1)
        
        # progress.set(0)
        #print(pi)
        print('Generating next sample')

        # Generate the next population
        xs = np.empty((n_pop, len(p0)), dtype=np.int64)

        # Precompute probabilities
        # Individual probabilities p(X_pi_i)
        p_inds = np.empty_like(pi, dtype=np.float64)
        for j in prange(len(pi)):
            total = 0.0
            for k in range(elite.shape[0]):
                total += elite[k, pi[j]]
            p_inds[j] = total / elite.shape[0] 
        
        # Joint probabilities p(X_pi_i, X_pi_(i-1)).
        p_joints = np.empty_like(pi, dtype=np.float64)
        p_joints[0] = 1
        for j in prange(len(pi)):
            # Joint probability p(X_i1, X_i2)
            total = 0.0
            for k in range(elite.shape[0]):
                total += elite[k, pi[j-1]] * elite[k, pi[j]]
            p_joints[j] = total / elite.shape[0]

        # debugging AAAA
        # print([p_joints[k+1] / p_inds[k] if (p_inds[k] != 0) else np.nan for k in range(len(p_inds)-1)])
        # print([(p_inds[k+1] - p_joints[k+1]) / (1 - p_inds[k]) if (p_inds[k] != 1) else np.nan for k in range(len(p_inds)-1)])

        for j in prange(n_pop):
            # Step 1: First probability
            xs[j, pi[0]] = np.random.binomial(1, p_inds[0])
            # Step 2: Chained probabilities
            for k in range(1, len(p0)):
                if xs[j, pi[k-1]] == 1:
                    # Conditional probability p(X_pi_i, x_pi_(i-1)=1)
                    p = p_joints[k] / p_inds[k-1] # in this case it must be that p_inds[k-1] > 0.
                else:
                    # Conditional probability p(X_pi_i, x_pi_(i-1)=0)
                    p = (p_inds[k] - p_joints[k]) / (1 - p_inds[k-1]) # in this case it must be that p_inds[k-1] < 1.
                p = round(p, 3)
                xs[j,pi[k]] = np.random.binomial(1, p)
...
if __name__ == '__main__':
    ...
    opt_dir = '/home/dataadmin/GBTData/SharedDataDirectory/lscx_081526/target_selection/082426'
    sefd = 10
    is_decay = True
    ...
    files = bank_info['time'].apply(lambda l: l + '.npy')
    ...
    # Convert to Fortran arrays to speed up asymmetry calculations
    uninj_norm_spectra = np.asfortranarray(uninj_norm_spectra)
    template_spectra = np.asfortranarray(template_spectra)

    # Optimization for three test objectives.
    asym_type = 'intensity'
    funcs = [objective_weighted_sum] #[objective_wammsd, objective_weighted_sum, objective_add]
    func_names = ['objective_weighted_sum'] #['objective_wammsd', 'objective_weighted_sum', 'objective_add']
    for func, func_name in zip(funcs, func_names):
        ...
        asym_dir = os.path.join(opt_dir, func_name)
        ...
        if os.path.exists(os.path.join(asym_dir, 'best_state.npy')):
            ...
            p0 = 0.5 * np.ones(len(bank_info)) # 0.5

            # Run optimization
            with ProgressBar(total=len(p0)) as progress:
                state_best, f_best, perfs = cross_entropy(#mimic(#cross_entropy(
                    func,
                    len(p0),
                    0.01,
                    1000,
        ...
        mad_aya = np.nanmedian(np.abs(asymmetry_aya - np.nanmedian(asymmetry_aya)))
        mad_best = np.nanmedian(np.abs(asymmetry_best - np.nanmedian(asymmetry_best)))
        ...
        ax.set_ylim(np.nanpercentile(asymmetry_aya, 2), np.nanpercentile(asymmetry_aya, 98))
        ax.legend()
        fig.savefig(os.path.join(asym_dir, 'best_asymmetry_plot.png'), dpi=300, bbox_inches='tight')
        np.save(os.path.join(asym_dir, 'best_asymmetry.npy'), np.array([freqs, asymmetry_best]))
        ...
        ax.set_title(f'inner={sum(inner)}, outer={sum(outer)}', fontsize=14)
