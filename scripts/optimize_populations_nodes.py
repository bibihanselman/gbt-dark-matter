...
def build_asymmetry(state, node=None):
    '''Form asymmetry from a given state.'''
    inner = state == 1
    outer = state == -1

    if node is not None:
        start = np.argmin(np.abs(xs - (freq_start + node*1024*freq_diff)))
        stop = np.argmin(np.abs(xs - (freq_start + (node+1)*1024*freq_diff)))
        specs = spectra[:, start:stop]
    else:
        specs = template_spectra
...
def objective_mad_new(state, node=0):
    '''The objective function: template peak / MAD.'''
    asymmetry = build_asymmetry(state, node=node)
    template = build_asymmetry(state)
...
def generate_movie(states, stds, node):
    ...
    def frame(idx):
        ...
        stop = np.argmin(np.abs(xs - (freq_start + (node+1)*1024*freq_diff)))
        asymmetry = build_asymmetry(state, node=node)
        asym.set_data(xs[start:stop], asymmetry)
    ...
    save_dir = os.path.join(opt_dir, f'node_{node}', 'movie.gif')
...
if __name__ == '__main__':
    ...
    xs_all = np.load('/home/dataadmin/GBTData/SharedDataDirectory/cband_052726/data/preprocessed/1/HIP11.npy')[0]
    freq_start = xs_all[0]
    freq_diff = xs_all[1] - xs_all[0]
    print(f'Freq start: {freq_start}, freq diff: {freq_diff}')
    ...
    def run(node):
        if os.path.exists(os.path.join(opt_dir, f'node_{node}', 'best_state.npy')):
            print('Optimization already complete')
        else:
            # Run optimization
            obj = partial(objective_mad_new, node=node)
            state_best, noise_best, states, stds, Ts = simulated_annealing(obj, state_init, 100_000, 1.0, 0.99, adaptive=True)

            # Save optimization results
            os.makedirs(os.path.join(opt_dir, f'node_{node}'), exist_ok=True)
            np.save(os.path.join(opt_dir, f'node_{node}', 'best_state.npy'), state_best)
            np.save(os.path.join(opt_dir, f'node_{node}', 'states.npy'), states)
            np.save(os.path.join(opt_dir, f'node_{node}', 'stds.npy'), stds)
            np.save(os.path.join(opt_dir, f'node_{node}', 'temps.npy'), Ts)
    ...
    def save_plots(node):
        state_best = np.load(os.path.join(opt_dir, f'node_{node}', 'best_state.npy'))
        states = np.load(os.path.join(opt_dir, f'node_{node}', 'states.npy'))
        stds = np.load(os.path.join(opt_dir, f'node_{node}', 'stds.npy'))

        # Save plot of best asymmetry
        asymmetry_best = build_asymmetry(state_best, node=node)
        fig, ax = plt.subplots()
        start = np.argmin(np.abs(xs - (freq_start + node*1024*freq_diff)))
        stop = np.argmin(np.abs(xs - (freq_start + (node+1)*1024*freq_diff)))
        ax.plot(xs[start:stop], asymmetry_best)
        ...
        fig.savefig(os.path.join(opt_dir, f'node_{node}', 'best_asymmetry_plot.png'), dpi=300, bbox_inches='tight')
        np.save(os.path.join(opt_dir, f'node_{node}', 'best_asymmetry.npy'), asymmetry_best)

        # Generate movie
        if movie:
            print('Generating movie...')
            generate_movie(states, stds, node)
            print('Movie saved!')
    
    with ThreadPoolExecutor(max_workers=8) as executor:
        results = executor.map(save_plots, range(8))
