...
def calculate_pval(normalize_func, freqs, data_dir=None, save_dir=None, dir_name=None, **kwargs):
    ...
    if skip:
        ...
        ### REPLACE WITH GPU NORMALIZATION - save normalized spectra in save_dir ###
...
if __name__ == '__main__':
    ...
    is_decay = case == 'decay'
    signal_sizes = [4e-28, 2e-28, 1e-28, 5e-29, 3e-29, 2e-29, 1e-29, 5e-30] if is_decay else [1e-24, 5e-25, 2e-25, 1e-25, 5e-26, 3e-26, 2e-26, 1e-26]
    template_signal_size = 1e-28 if is_decay else 1e-25
    
    root = '/home/dataadmin/GBTData/SharedDataDirectory/xband_072625'
    analysis_dir = f'analyses/8000_8500_mhz/{case}'
    ...
    if os.path.exists(uninj_save_dir):
        ...
        ### REPLACE WITH GPU NORMALIZATION - save normalized spectra in uninj_save_dir ###
        with mp.Pool(processes=num_processes) as p:
            normalize_func_partial = partial(normalize_dynamic,
    ...
    if os.path.exists(temp_save_dir):
        ...
        with mp.Pool(processes=num_processes) as p:
            ### REPLACE WITH GPU NORMALIZATION - save normalized spectra in temp_save_dir ###
            normalize_func_partial = partial(normalize_dynamic,
    ...
    for start_freq in tqdm(start_freqs):
        ...
        freqs = np.arange(start_freq, stop, spacing)
        analyze_partial = partial(analyze, start_freq=start_freq)
